...
const list = (req, res) => {
  ...
  MovieModel.find((err, result) => {
    ...
    //res.json(result);
    res.render("movie/list", { result }); //루트가 views이기 때문에 바로 music/list하면돼
  })
...
const create = (req, res) => {
  const { title, director, year } = req.body; //app.js에서 bodyparser가 설정되어있어서 사용가능
...
const showCreatePage = (req, res) => {
  res.render("movie/create"); //ejs를 넘겨줌
};
